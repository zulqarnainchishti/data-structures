...
void test_insert()
{
    ...
    printf("Expected: <10> <20> <25> <30> <40> <50>\n");
    printf("Actual:   ");
    inorder(root);
    printf("\n");

    printf("Test 2.2: Height of the tree after insertions\n");
    printf("Expected: 2\n");
    printf("Actual:   %d\n", height(root));
    if (height(root) != 2)
        TEST_FAIL();

    printf("Test 2.3: Insert duplicate value (20) and check size\n");
...
void test_min_max_successor_predecessor()
{
    ...
    printf("Expected: 50\n");
    AVLTree *succ_40 = successor(root, 40);
    ...
    printf("Expected: NULL\n");
    AVLTree *succ_80 = successor(root, 80);
    ...
    printf("Expected: 50\n");
    AVLTree *pred_60 = predecessor(root, 60);
    ...
    printf("Expected: NULL\n");
    AVLTree *pred_20 = predecessor(root, 20);
...
void test_tree_properties()
{
    ...
    printf("Expected: isPerfect=true, isComplete=true, isFull=true\n");
    printf("Actual:   isPerfect=%s, isComplete=%s, isFull=%s\n", isPerfect(perfect_tree) ? "true" : "false", isComplete(perfect_tree) ? "true" : "false", isFull(perfect_tree) ? "true" : "false");
    if (!isPerfect(perfect_tree) || !isComplete(perfect_tree) || !isFull(perfect_tree))
        TEST_FAIL();
    destroy(perfect_tree);

    AVLTree *full_and_complete_tree = NULL;
    ...
    AVLTree *complete_but_not_full_tree = NULL;
    ...
    printf("Test 8.4: Symmetric tree properties\n");
    ...
    printf("Test 8.5: Insertions that cause rebalancing\n");
    AVLTree *rebalance_tree = NULL;
    rebalance_tree = insert(rebalance_tree, 30);
    rebalance_tree = insert(rebalance_tree, 20);
    rebalance_tree = insert(rebalance_tree, 10);
    printf("Expected root: 20, Left: 10, Right: 30\n");
    printf("Actual root: %d, Left: %d, Right: %d\n", rebalance_tree->data, rebalance_tree->left->data, rebalance_tree->right->data);
    if (rebalance_tree->data != 20 || rebalance_tree->left->data != 10 || rebalance_tree->right->data != 30)
        TEST_FAIL();
    destroy(rebalance_tree);
...
void test_floor_ceiling()
{
    ...
    printf("Expected: 40\n");
    AVLTree *ceil_node = ceiling(root, 35);
    ...
    printf("Expected: 30\n");
    AVLTree *floor_node = floored(root, 35);
    ...
    printf("Expected: NULL\n");
    AVLTree *ceil_90 = ceiling(root, 90);
    ...
    printf("Expected: NULL\n");
    AVLTree *floor_10 = floored(root, 10);
...
int main()
{
    printf(ANSI_COLOR_BLUE "\n*** Starting extensive AVL Tree test suite ***\n\n" ANSI_COLOR_RESET);

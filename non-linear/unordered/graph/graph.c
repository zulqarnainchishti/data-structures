...
int mapIndex(Graph graph, char vertex)
{
...
Graph copy(Graph graph)
{
    ...
    newGraph.V = graph.V;
    newGraph.E = graph.E;
    newGraph.labelMap = (char *)realloc(newGraph.labelMap, newGraph.V * sizeof(char));
    newGraph.adjacencyList = (Edge **)realloc(newGraph.adjacencyList, newGraph.V * sizeof(Edge *));

    for (int i = 0; i < graph.V; i++)
    {
        newGraph.labelMap[i] = graph.labelMap[i];
...
void removeVertex(Graph *graph, char vertex)
{
...
void addEdge(Graph *graph, char vertex1, int edge, char vertex2)
{
...
void removeEdge(Graph *graph, char vertex1, char vertex2)
{
    int idx1 = index(*graph, vertex1);
    int idx2 = index(*graph, vertex2);
    if (idx1 == -1 || idx2 == -1)
...
bool hasSelfloop(Graph graph, char vertex)
{
    int idx = index(graph, vertex);
    if (idx == -1)
        return false;
...
bool areAdjacent(Graph graph, char vertex1, char vertex2)
{
...
int vertexDegree(Graph graph, char vertex)
{
    int idx = index(graph, vertex);
    if (idx == -1)
        return -1;
...
int edgeWeight(Graph graph, char vertex1, char vertex2)
{
    int idx = index(graph, vertex1);
    if (idx == -1)
        return -1;
...
float graphDensity(Graph graph)
{
    return (float)(2 * graph.E) / (graph.V * (graph.V + 1));
}

void DFSIterative(Graph graph, char vertex)
{
...
void dfsHelper(Graph graph, int index, bool visited[])
{
    visited[index] = true;
    printf("%c ", graph.labelMap[index]);
    Edge *curr = graph.adjacencyList[index];
...
void DFSRecursive(Graph graph, char label)
{
...
void BFSIterative(Graph graph, char label)
{
    int index = mapIndex(graph, label);
    if (index == -1)
        return;
...
void bfsHelper(Graph graph, int currLevel[], int currCount, bool visited[])
{
...
void BFSRecursive(Graph graph, char label)
{
    int index = mapIndex(graph, label);
    if (index == -1)
        return;
...
bool isSparse(Graph graph)
{
    return density(graph) <= 0.5;
}

bool isDense(Graph graph)
{
    return density(graph) > 0.5;
}
...
bool isDisconnected(Graph graph)
{
